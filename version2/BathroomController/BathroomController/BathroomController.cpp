	...
	{			
		if (door_event_flag == YES)
		{
			if (CHECK_DOOR_OPENED)
			{
				...
				if (CHECK_ANY_LIGHT_ON)
				{
					light(ON); // зажигаем свет ещё раз, чтобы переключить реле в необходимом порядке
				}				
			} else
			{
				// обновить счетчик
...
static void onDoorEvent()
{
	...
	if (CHECK_DOOR_OPENED)	
	{
		uart_puts_P(msg_door_opened);
	} else 
	{
		uart_puts_P(msg_door_closed);
	}
	
	check_relax_light_mode(true);
