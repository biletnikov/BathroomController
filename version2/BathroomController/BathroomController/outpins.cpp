...
struct PinConfig 
{
	uint8_t out_indx; // логический индекс пина
	uint8_t reg_pin; // номер пина в регистре
	volatile uint8_t * reg_portin; // регистр содержит значения порта
	volatile uint8_t * reg; // регистр
	uint8_t logic_level; // логическая конфигурация HIGH Level = ON или LOW
};


// если конфиг не найден, вернуть 0
uint8_t find_pin_config(uint8_t out_index, PinConfig * conf, uint8_t out_pin_level_config)
{
...
extern uint8_t set_output(uint8_t output_index, uint8_t on_value, uint8_t out_pin_level_config)
{
	...
	if (conf.logic_level)
	{
		...
		// Включение по высокому логическому уровню
		if (on_value == PIN_ON)
		{
	...
	{
		// Включение по низкому логическому уровню
		if (on_value == PIN_ON)
		{			
			*(conf.reg)&=~(1<<conf.reg_pin);
		}
